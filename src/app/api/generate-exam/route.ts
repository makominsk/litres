...
- "multiple": 4 варианта, СТРОГО один правильный
- "date": назвать что произошло в указанную дату (только если в тексте есть конкретные даты)
...
КРИТИЧЕСКИ ВАЖНЫЕ правила для multiple:
1. Вопрос должен иметь РОВНО ОДИН правильный ответ среди 4 вариантов — если правильных вариантов может быть несколько, используй тип "open" вместо "multiple"
2. correctAnswer должен совпадать ДОСЛОВНО (побуквенно) с одним из элементов массива options
3. Остальные 3 варианта должны быть ЯВНО неправильными — не допускай вариантов, которые тоже могут считаться правильными
4. Не задавай вопросы о географии, если правильных ответов может быть несколько (например "на каком континенте" — если цивилизации были на двух континентах)
5. Спрашивай о конкретных фактах: имена, даты, названия, определения — то, что имеет единственный точный ответ

Общие правила:
- Все вопросы и ответы на русском языке
- Вопросы чёткие, понятные для 10-летнего ребёнка
- Для multiple: correctAnswer — это краткая фраза (2-5 слов), совпадающая дословно с одним из options`
